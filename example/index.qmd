...
```{r}
#| eval: false
if (file.exists("wf_set_fit.rda")) {
  eval_fits <- FALSE
  # read outputs in from source:
  load("wf_set_fit.rda")
  load("metrics_wf_set.Rda")
  load("metrics_mars.Rda")
  load("final_fit_bundled.Rda")
  final_fit <- bundle::unbundle(final_fit_bundled)
} else {
  eval_fits <- TRUE
}
```
...
With a [multi-core setup registered](https://tune.tidymodels.org/articles/extras/optimizations.html), tidymodels will now make use of all of the cores on my computer for expensive computations.
...
```{r load-msnflights22}
load("msnflights22.rda")

msnflights22
```

::: callout-note
You can make your own flights data using the [anyflights](https://simonpcouch.github.io/anyflights/) package! The [`query_data.R`](example/query_data.R) file contains the code used to generate this dataset.
:::

We'd like to model `delayed`, a binary outcome variable giving whether the flight was delayed by 10 or more minutes.
...
Now that we've defined each of our modeling configurations, we'll fit them to the resamples we defined earlier.
...
metrics_mars <- collect_metrics(mars_sim_anneal_fit, summarize = FALSE)
```
...
#| eval: !expr eval_fits
#| include: false
final_fit_bundled <- bundle(final_fit)
save(final_fit_bundled, file = "final_fit_bundled.rda")
```
...
```{r}
#| eval: false
final_fit_vetiver <- vetiver_model(final_fit, "flights")
