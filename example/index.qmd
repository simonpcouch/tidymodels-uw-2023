...
if (file.exists("wf_set_fit.rda")) {
  ...
  load("metrics_mars_sim_anneal_fit.rda")
  load("mars_final_fit.rda")
} else {
  eval_fits <- TRUE
}
```
...
It looks like a [bagged MARS model](https://parsnip.tidymodels.org/reference/bag_mars.html) with centered and scaled predictors was considerably more performant than the other proposed models. Let's work with those MARS results and see if we can make any further improvements to performance:
